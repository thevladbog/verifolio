...
1. Root `package.json`/`package-lock.json`/`node_modules` are accidental (see CRITICAL) —
   delete, do not commit.
2. `/dashboard` doesn't exist yet — `auth/callback` redirects there (login dead-ends
   on 404). Expected to close with Task 8; verify before merge.
3. Plan Task 4 Step 2 deviation: no fresh-profile onboarding (design 8a — empty
...
1. `requests/[id]/page.tsx:114` renders `null` on 403/404 — add a not-found state.
2. `lib/requests/queries.ts:38` `useContactNames` caps at 4 pages — builder picker
   silently omits contacts beyond ~200; needs search/fallback eventually.
3. `lib/requests/status.ts:36-38` — SUBMITTED/NEEDS_REVIEW use the `locked` badge
   variant; semantically the document-lock signal. Revisit in Task 12 fidelity pass.
4. `NEXT_PUBLIC_REGION_LINKS` (landing) missing from `.env.example`.
5. Task 4 plan said dark-ink sidebar; implementation is a top header nav ("per design
   canvas") — fine if the design confirms it, document the deviation.
6. Task 6 components implemented inline in `requests/new/page.tsx` instead of the
